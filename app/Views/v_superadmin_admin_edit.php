...
<div class="form-container">
    <h2>Edit Admin</h2>
    <?= form_open(base_url() . 'superadmin/update_admin'); ?>

    <div class="form-group">
        <label for="nama"><i class="fas fa-user"></i>Nama</label>
        <input type="text" id="nama" name="nama" value="<?= $data_admin['nama'] ?>" required>
    </div>

    <div class="form-group">
        <label for="nama"><i class="fas fa-user"></i>Jenjang</label>
        <select id="jenjang" name="jenjang" required>
            <option <?= ($data_admin['jenjang'] == "SD") ? "selected" : "" ?> value="SD">SD</option>
            <option <?= ($data_admin['jenjang'] == "SMP") ? "selected" : "" ?> value="SMP">SMP</option>
            <option <?= ($data_admin['jenjang'] == "SMA") ? "selected" : "" ?> value="SMA">SMA</option>
        </select>
    </div>

    <div class="form-group">
        <label for="email"><i class="fas fa-envelope"></i>Email</label>
        <input type="email" id="email" name="email" value="<?= $data_admin['email'] ?>" required>
        <small class="text-danger" id="emailError"></small>
    </div>

    <input type="hidden" name="id_admin" value="<?= $data_admin['id_user'] ?>">
...
<script>
    var ijinajax = true;
    var emaillama = "<?= $data_admin['email'] ?>";
